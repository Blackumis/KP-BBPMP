...
const ListEvents = () => {
  ...
  // Fungsi untuk mendapatkan status aktual (memperhitungkan mulai_waktu_absensi)
  const getActualStatus = (event) => {
    ...
    if (start && now < start) {
      return event.status === "active" ? "draft" : event.status;
    }
    if ((start ? now >= start : true) && (deadline ? now <= deadline : true)) return "active";
    return event.status;
  };
  ...
  return (
    <>
            ...
            <table className="min-w-full bg-white table-fixed">
              ...
              <tbody className="divide-y divide-gray-200">
                {sortedEvents.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((event, index) => {
                  ...
                  const start = event.mulai_waktu_absensi ? new Date(event.mulai_waktu_absensi) : null;
                  const now = new Date();
                  const startsIn = start && now < start ? getTimeRemaining(event.mulai_waktu_absensi) : null;
                  const endsIn = event.batas_waktu_absensi ? getTimeRemaining(event.batas_waktu_absensi) : null;
                    ...
                    <tr key={event.id} className={`hover:bg-gray-50 transition duration-150 ${isExpired ? "bg-orange-50" : ""}`}>
                      ...
                      <td className="py-4 px-4 text-sm text-center align-middle">
                        {event.batas_waktu_absensi ? (
                          !isExpired ? (
                            startsIn ? (
                              <span className="inline-flex items-center px-3 py-1 rounded-md text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200">{startsIn}</span>
                            ) : (
                              <span className="inline-flex items-center px-3 py-1 rounded-md text-xs font-semibold bg-blue-50 text-blue-700 border border-blue-200">{endsIn}</span>
                            )
                          ) : (
      ...
      {/* SMTP Settings Modal */}
      <SmtpSettingsModal isOpen={showSmtpSettings} onClose={() => setShowSmtpSettings(false)} />
    </>
  );
};

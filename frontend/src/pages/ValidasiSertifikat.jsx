  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/certificates/download/${encodedCertNumber}`
      );
  ...
  useEffect(() => {
    const validateCertificate = async () => {
      try {
        ...
        const encodedCertNumber = encodeURIComponent(certificateNumber);
        const response = await fetch(
          `http://localhost:5000/api/certificates/validate/${encodedCertNumber}`
        );
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="text-lg font-mono font-semibold text-gray-900">
                  {validationData?.certificate?.nomor_sertifikat}
                </p>
              </div>
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm ${
                    validationData?.certificate?.status === 'sertifikat_terkirim'
          ...
          <div className="px-6 py-4">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-lg font-semibold text-gray-900">
                  {validationData?.event?.nama_kegiatan}
                </p>
              </div>
              <div>

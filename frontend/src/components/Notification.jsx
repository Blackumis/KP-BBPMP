...
const Notification = ({ message, type = "info", duration = 2000, onClose }) => {
  ...
  return (
    ...
    >
      <div className={`${style.bg} ${style.border} border-2 rounded-xl shadow-2xl p-4 pr-12 min-w-75 max-w-md relative backdrop-blur-sm`}>
        <div className="flex items-start gap-3">
          ...
          <div className={`flex-1 ${style.text} font-medium text-sm leading-relaxed`}>{message}</div>
        </div>
        <button
          onClick={() => {
            setIsExiting(true);

...
export const createOfficial = async (req, res) => {
  try {
    ...
    const [result] = await db.query(
      `INSERT INTO officials (name, position, signature_image_path, signature_qr_path, is_active, created_by) 
       VALUES (?, ?, ?, ?, ?, ?)`,
    ...
    const [newOfficial] = await db.query(`SELECT * FROM officials WHERE id = ?`, [result.insertId]);
...
export const updateOfficial = async (req, res) => {
  try {
    ...
    const [updatedOfficial] = await db.query(`SELECT * FROM officials WHERE id = ?`, [id]);
...
export const deleteOfficial = async (req, res) => {
  try {
    ...
    await db.query(`DELETE FROM officials WHERE id = ?`, [id]);

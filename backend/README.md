...
#### Attendance Tokens (Admin)

Admin can generate opaque attendance tokens (JWT containing only `activity_id`) via the protected endpoint:

`GET /api/attendance/generate-token/:event_id` (requires admin auth)

The response contains `{ token }` which the frontend should use to build public links such as `/form/:token` and `/submit/:token`.

#### GET /api/attendance/form/:token

Get form kegiatan (public - untuk user). The public routes no longer expose event id or name in the URL. Use an opaque JWT token generated by an admin to access the form.
...
Submit absensi (public - untuk user). Use the same opaque token as provided with the public form.
